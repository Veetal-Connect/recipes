...
export async function build() {
  const [reputation, reviews] = await Promise.all([
    veetalOptional(`/feed/accommodation/${SLUG}/reputation?provider=google`),
    // include_competitors defaults to TRUE. Forget it and your hotel's own website
    // starts showing your competitors' reviews.
    veetal(
      `/feed/accommodation/${SLUG}/reviews?provider=google&include_competitors=false&limit=50`,
    ),
  ]);

  const own = ((reputation && reputation.accommodation) || [])[0] || {};

  return {
    name: own.accommodation_name ?? null,
    score: own.review_score ?? null,
    count: own.review_count ?? null,
